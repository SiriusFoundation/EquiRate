function  [Avg,Results]=Metrics(Dataset, Predictions, m_TopN)
format short g
% ----Input ----
...
PredMatrix=zeros(size(Dataset,1),size(Dataset,2));
for row=1:size(Predictions,1)
	UserID=Predictions(row,1);
	ItemID=Predictions(row,2);
	Rating=Predictions(row,3);
	PredMatrix(UserID,ItemID)=Rating;
end


%% Recommendations for individuals
for i=1:size(Dataset,1)
    [val,idx]=maxk(PredMatrix(i,:),10);
    TopN(i,:)=idx;
end
